...
public:
    Point2D A, B, C, D;
    Tetragon(Point2D a, Point2D b, Point2D c, Point2D d) throw (PointExeption): A(a), B(b), C(c), D(d) {
...
public:
    ...
    Rectangle(Point2D start, double a, double b): Tetragon(start, Point2D(start.x+a, start.y), Point2D(start.x+a, start.y+a), Point2D(start.x+a, start.y+a)){ short_side_= min(a, b); long_side_ = max(a, b);};
